...
const MesReservationsProp = () => {
  ...
  const handleReponseReservation = async (id, statut) => {
    ...
    try {
      ...
      // NOUVEAU : Recalculer les conflits juste avant la confirmation
      const reservationToConfirm = reservations.find((res) => res._id === id);
      const actualConflicts =
        statut === "confirmée"
          ? checkConflictingReservations(reservationToConfirm)
          : [];
      ...
      // MODIFIÉ : Utiliser les conflits recalculés
      setReservations(
        reservations.map((res) => {
          // Mettre à jour la réservation confirmée/refusée
          if (res._id === id) {
          ...
          // Mettre à jour les réservations en conflit si une réservation est confirmée
            ...
            statut === "confirmée" &&
            actualConflicts.some((conflict) => conflict._id === res._id)
          ) {
            return { ...res, statut: "annulée" };
          }
          // Garder les autres réservations inchangées
  ...
  // 1. Ajoutez cette fonction pour vérifier si une réservation confirmée peut être annulée (dans les 48h)
  const canCancelConfirmedReservation = (reservation) => {
    if (reservation.statut !== "confirmée") return false;
    ...
    return hoursDiff <= 48; // 48 heures
  };
  // 2. Ajoutez cette fonction pour gérer l'annulation d'une réservation confirmée
  const handleCancelConfirmedReservation = async (reservationId) => {
    if (processing) return;

    const confirmation = window.confirm(
      "Êtes-vous sûr de vouloir annuler cette réservation confirmée ? Les autres demandes en attente pour ces dates redeviendront disponibles."
    );

    if (confirmation) {
      try {
        setProcessing(true);
        await reservationService.annulerReservationConfirmee(reservationId);

        toast.success(
          "Réservation annulée avec succès. Les demandes en attente sont à nouveau disponibles."
        );
            ...
            {filteredReservations.map((reservation) => (
              ...
              >
                ...
                {reservation.statut === "confirmée" &&
                  canCancelConfirmedReservation(reservation) && (
                    <div className="payment-warning">
                      <i className="fas fa-exclamation-triangle"></i>
                      <span>
                        En attente de paiement - Vous pouvez annuler si non payé
                      </span>
                    </div>
                  )}
                <div className="reservation-actions">
                  ...
                  {/* NOUVEAU : Bouton d'annulation pour les réservations confirmées */}
                  {reservation.statut === "confirmée" &&
                    canCancelConfirmedReservation(reservation) && (
                      <button
                        className="action-button cancel-confirmed"
                        onClick={() =>
                          handleCancelConfirmedReservation(reservation._id)
                        }
                        disabled={processing}
                        title="Annuler (client n'a pas payé)"
